...
class HoaDon{
    ...
    public:
        ...
        friend istream& operator >> (istream& in, HoaDon &hd) {
            ...
            hd.bill_id = "HD0" + temp;
            countHoaDon++;
            hd.profit = hd.quantity * MH[hd.hanghoa_id].sell_price - MH[hd.hanghoa_id].buy_price * hd.quantity;
            return in;
        }
        friend ostream& operator << (ostream& out, HoaDon hd) {
            out << hd.bill_id << " " << KH[hd.customer_id].customer_name << " " << KH[hd.customer_id].customer_address << " " << MH[hd.hanghoa_id].hanghoa_name << " "  << hd.quantity  << " " << MH[hd.hanghoa_id].sell_price * hd.quantity  << " " << hd.profit << endl;
            return out;
        }
};
